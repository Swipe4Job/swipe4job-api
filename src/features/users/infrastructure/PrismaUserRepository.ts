...
import {ByUserID} from "../domain/ByUserID";
    ...
    async search(criteria: UserCriteria): Promise<Array<User> | undefined> {
        ...
        return result.map(entry => {
            return new User({
                ...
                walletAddress: entry.wallet_address ? new WalletAddress(entry.wallet_address) : undefined,
                phoneNumber: new PhoneNumber(entry.phone_number),
                password: entry.password ? UserPassword.from(entry.password) : undefined,
